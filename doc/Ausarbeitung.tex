...
%\begin{description}[font=\normalfont]
    ...
    \subparagraph{getPoints()} 
    Simpler getter für die Punkte des Graphen.
    \subparagraph{setPoints(points)} 
    ...
    \subparagraph{eraseAllAnnotations()} 
    Jegliche Annotations aller Algorithmen werden entfernt.
    \subparagraph{eraseAnnotations(algoName)} 
    Nur die Annotations, die zuvor unter \texttt{algoName} gezeichnet wurden, werden gelöscht.
    \subparagraph{initAlgoBox(algoName, color)} 
    \subparagraph{setAlgoBox(algoName)} 
    \subparagraph{setAlgoBoxLoading(algoName)} 
    \subparagraph{refreshAlgoBox(algoName, score, info, annots, color} 
    \subparagraph{addLevelToNav(levelName)} 
    \subparagraph{changePageHeader(text)} 
    \subparagraph{showHighscore(data)} 
    \subparagraph{clearHighscore()} 
    \subparagraph{setHighscoreVisibility(visible)} 
    \subparagraph{showGraph()} 
    \subparagraph{getGraph()} 
    \subparagraph{isActive(algoName)} 
    \subparagraph{checkTmpBestval(val)} 
%\end{description}
