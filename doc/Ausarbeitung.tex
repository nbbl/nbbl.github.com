...
\begin{description}[font=\normalfont]
  \item{\lststyle{clone()}}
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{abs()}} 
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{inv()}}
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)} 
  \item{\lststyle{v1.add(v2)}} 
    Addiert die Vektoren v1 und v2.
  \item{\lststyle{v1.degreesTo(v2)}} 
    Berechnet den Winkel zwischen den Vektoren v1 und v2.
  \item{\lststyle{v1.distance(v2)}} 
    Berechnet die euklidische Distanz zwischen v1 und v2 (als Ortsvektoren). 
  ...
  \item{\lststyle{interpolate(v,f)}} 
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{v.length()}}
    euklidische Norm des Vektors.
  \item{\lststyle{normalize(thickness)}}
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{orbit(ORIGIN,arcWidth,arcHeight,degrees}}
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)} 
  \item{\lststyle{v.moveTo(coordX,coordY)}}
    Ändert die Koordinaten des Vektors in die übergebenen ab.
  \item{\lststyle{offset(dx,dy)}} 
        \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{substract(v)}} 
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
...
\begin{description}[font=\normalfont]
  \item{\lststyle{interpolate(pt1,pt2,f)}} 
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)}
  \item{\lststyle{polar(len,angle)}} 
    \textbf{(Haben wir anscheinend nie benutzt, falls doch hier ergänzen)} 
...
\begin{description}[font=\normalfont]
  \item{\lststyle{pt1.isAdjacentTo(pt2)}}
    Iteriert durch die inzidenten Kanten von pt1 und überprüft durch \lststyle{getAdjacent}    (siehe Kantenfunktionen {\bf steht da noch nicht!}) ob pt2 in einer von diesen adjazent zu pt1 ist.   
\end{description}
...
\begin{description}[font=\normalfont]
  ...
  \item{\lststyle{e.projectionToEdge(pt)}}
    Analog zu \lststyle{ProjectionToLine}, liefert jedoch \lststyle{null} sollte die Projektion nicht in \textbf{Alle Kanten benennen!} liegen.
  \item{\lststyle{e.distanceToLine(pt)}}
    Liefert den Abstand eines Punktes pt zu der Geraden die durch die Endpunkte von e verläuft.
